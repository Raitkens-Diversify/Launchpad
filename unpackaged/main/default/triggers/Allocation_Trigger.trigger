trigger Allocation_Trigger on Allocation__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
	Allocation_TriggerHandler handler = new Allocation_TriggerHandler();
	if (Trigger.isBefore) {
		 if (Trigger.isInsert) {
