trigger Check_Log_Trigger on Check_Log__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
	Check_Log_TriggerHandler handler = new Check_Log_TriggerHandler();
	if (Trigger.isBefore) {
		 if (Trigger.isInsert) {
