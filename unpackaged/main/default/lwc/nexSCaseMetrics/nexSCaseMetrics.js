...
import getCaseMetrics from "@salesforce/apex/NexSCaseMetricsController.getCaseMetrics";
...
export default class NexSCaseMetrics extends LightningElement {
  scope = SCOPE_MY;
  segments = [];
  totalCount = 0;
  errorMessage = "";
  ...
  connectedCallback() {
    if (!this._stylesLoaded) {
      ...
      loadStyle(this, diversifyStyles).catch((error) => {
        ...
        console.error("[nexSCaseMetrics] Failed to load diversifyStyles", error);
      });
    }
    ...
    loadScript(this, chartjs)
      ...
      .catch((error) => {
        // eslint-disable-next-line no-console
        console.error("[nexSCaseMetrics] Failed to load Chart.js", error);
      });
  }
  ...
  wiredCaseMetrics({ data, error }) {
    ...
    if (error) {
      ...
      this._renderSignature = "";
      // eslint-disable-next-line no-console
      console.error("[nexSCaseMetrics] Failed to load case metrics", error);
    }
  }
